...
def dist_cal(latent_space_train, pos, dim = 2):
...
def result():
    if request.method == 'POST':
        ...
        for i in range(top_num+1):
            dis.append(latent_space_dis[latent_space_train_seq[i]])
            lis_num.append(latent_space_train_seq[i])
            req_y_pre.append(round(float(p_prediction[latent_space_train_seq[i]]), 4))
            req_y_ture.append(y_train[latent_space_train_seq[i]])
            req_smiles.append(out_raw[latent_space_train_seq[i]])
        return render_template("result.html",top_num = top_num,
            dis = dis,
            lis_num = lis_num,
            req_y_pre = req_y_pre,
            req_y_ture = req_y_ture,
            req_smiles = req_smiles
            )
'''
@app.route('/result',methods = ['POST', 'GET'])
def result():
...
'''
@app.route("/plot_png/<int:top_num>")
def plot_png(top_num):
    ...
    sc = ax.scatter(latent_train_list['PC1'], latent_train_list['PC2'], c = p_prediction, cmap = 'winter', vmin = -0.2, vmax = 0.2)

    ax.scatter(min_p[0,0], min_p[0,1], c = 'red', marker = 'x', s = 150) #-0.337
    
    ax.annotate('min', (min_p[0,0], min_p[0,1]),color='yellow') 
    for i in range(int(top_num)):
        ax.scatter(nei_t5[i,0], nei_t5[i,1], c = 'red', marker = '.', s = 150    )
        ax.annotate(i, (nei_t5[i,0], nei_t5[i,1]),color='yellow') 
