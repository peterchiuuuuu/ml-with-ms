...
def plot_png(homo_desire, lumo_desire, ramdom_sample_value, dis, std):
    ...
    pd_desire = pd.concat([pd_desire, pd.DataFrame(desire_homo_prediction), pd.DataFrame(desire_lumo_prediction), pd.DataFrame(predict_st), pd.DataFrame(final_prediction)], axis = 1)
    pd_desire.columns = ['PCA1', 'PCA2', 'desire_homo_prediction', 'desire_lumo_prediction', 'smiles', 'final_prediction']
    ...
    ax1.annotate(round(float(prediction_homo[highest_desire_value_index][0]), 3), (opti[0,0], opti[0,1]), horizontalalignment='right',color='black') 
